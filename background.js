...
const DEFAULT_SETTINGS = {
  ...
  // key 支持 "选择器@属性" 写法（content.js 合并时编译成函数）；出厂自带 IT 之家两条做示范
  customRules: [
    {
      name: "IT之家 · 首页列表",
      host: "ithome.com",
      item: "ul.nl li", // 每条新闻的 li（lapin 商业推广也在里面，让 Jev 照标题）
      title: "a",
      link: "a",
      key: "a@href", // 链接没变就不重照
      anchor: "a", // 徽标插在标题后面
      fold: "collapse" // 列表行级结构：折成一行最自然
    },
    {
      name: "IT之家 · 文章页",
      host: "ithome.com",
      item: "#dt .fl.content", // 正文区（首页没有 #dt，两条规则互不打扰）
      author: "#author_baidu strong",
      title: "h1",
      text: "#paragraph",
      key: "h1", // 标题就是指纹
      anchor: ".info .l", // 徽标插在作者/责编那行后面
      body: "#paragraph", // 模糊正文
      veil: "self", // 提示浮在文章区中间
      fold: "blur" // 用户主动点开的：模糊 + 提示，不折掉
    }
  ],
  // 屏蔽文章列表规则（独立功能，不进照妖）：撞到关键词的列表条目直接按 style 处理。
  // text 留空 = 用整条文字匹配；style: hide(隐藏) | blur(模糊) | fade(淡化)；keywords 空 = 规则躺着不动
  blockListRules: [
    {
      name: "IT之家 · 屏蔽列表",
      host: "ithome.com",
      ...
      text: "a", // 标题文字在 a 里
      style: "hide",
      keywords: [] // 出厂不带词：在弹窗里点「示范 · IT之家列表」，自己填要屏的词
    }
  ]
};
